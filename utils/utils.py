...
def check_permissions(required_permission="admin"):
    def real_decorator(func):
        ...
        def wrapped(update: Update, context: CallbackContext, *args, **kwargs):
            ...
            elif required_permission in ("w", "write") and (
                permissions["read"] and permissions["write"]
            ):
                return func(update, context, *args, **kwargs)
            elif required_permission in ("e", "edit") and (
                permissions["read"] and permissions["edit"]
            ):
                return func(update, context, *args, **kwargs)

            # all the permissions are disabled: unauthorized access
            logger.info(
                "unauthorized command usage (%s) by %d (%s)",
                required_permission,
                user_id,
                update.effective_user.first_name,
            )
            if update.callback_query:
                text = (
                    f'"{required_permission}" permission disabled for non-admin users'
                )
...
def failwithmessage(func):
    ...
    def wrapped(update: Update, context: CallbackContext, *args, **kwargs):
        try:
            ...
            error_str = str(e)
            logger.info(
                "error while running handler callback: %s", error_str, exc_info=True
            )
            ...
            if update.callback_query and error_str.lower().startswith(
                "query is too old"
            ):
    ...
    def wrapped(update: Update, context: CallbackContext, *args, **kwargs):
        try:
            ...
            if "not modified" not in str(err).lower():
                ...
                update.callback_query.answer("已是最新")
...
def failwithmessage_job(func):
    ...
    def wrapped(context: CallbackContext, *args, **kwargs):
        try:
            ...
            text = (
                f"#{context.bot.username} exception: an error occurred while running the job "
...
def hash_from_magnet(magnet_link: str):
    torrent_hash = re.search(
        r"magnet:\?xt=urn:btih:([a-z0-9]+)(?:&.*)?", magnet_link, re.I
    ).group(1)
