...
def on_add_or_remove_tags_command(update: Update, context: CallbackContext):
    ...
    if action == "+":
        text = (
            f"Tags added to <b>{torrent.name_escaped}</b>: <code>{tags_list_str}</code> "
            f'[<a href="{torrent.info_deeplink}">info</a>]'
        )
        torrent.add_tags(tags_list)
    else:
        text = (
            f"Tags removed from <b>{torrent.name_escaped}</b>: <code>{tags_list_str}</code> "
            f'[<a href="{torrent.info_deeplink}">info</a>]'
        )
...
updater.add_handler(
    MessageHandler(
        Filters.regex(r"^(\+|\-)(.+)") & Filters.reply, on_add_or_remove_tags_command
    )
)
