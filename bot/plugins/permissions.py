...
def set_permission(update: Update, context: CallbackContext):
    ...
    if len(context.args) < 2:
        update.message.reply_html(
            "使用方法: /pset <code>[permission key] [true/false/1/0]</code>"
        )
        return
    ...
    if val.lower() not in ("true", "false", "0", "1"):
        update.message.reply_html(
            "错误的值. 使用方法: /pset <code>[permission key] [true/false/1/0]</code>"
        )
        return

    if permissions.get(key, None) is None:
        update.message.reply_text("错误的 KEY. 使用 /permissions 查看权限配置")
        return
...
updater.add_handler(
    CommandHandler(["permissions", "p"], get_permissions),
    bot_command=BotCommand("permissions", "权限配置"),
)
updater.add_handler(
    CommandHandler(["pset"], set_permission), bot_command=BotCommand("pset", "权限变更")
)
