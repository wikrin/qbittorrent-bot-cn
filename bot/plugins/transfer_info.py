...
• 当前会话分享率: {session_share_rateo}

<b>Torrents 排队</b>
• 最大活跃数: {queueing_max_active_downloads} 下载, {queueing_max_active_uploads} 上传, \
{queueing_max_active_torrents} 合计
• 慢速 torrent 不计入限制内: {queueing_count_slow_torrents}
• 下载速度阈值: {queueing_slow_torrent_down_threshold} kb/s
• 上传速度阈值: {queueing_slow_torrent_up_threshold} kb/s

<b>做种限制</b>
• 达到设置的分享率: {max_ratio_enabled} (最大分享率: {max_ratio})
• 达到总做种时间: {max_seeding_time_enabled} (最大做种时间: {max_seeding_time} 分钟, \
然后 <a>{max_ratio_act}</a> Torrents)"""
...
def get_speed_text():
    ...
    fdict["global_down_limit"] = (
        u.get_human_readable(preferences["dl_limit"])
        if preferences["dl_limit"]
        else "无限制"
    )
    fdict["global_up_limit"] = (
        u.get_human_readable(preferences["up_limit"])
        if preferences["up_limit"]
        else "无限制"
    )

    fdict["alt_speed_status"] = "开" if qb.get_alternative_speed_status() else "关"
    fdict["alt_speed_down"] = (
        u.get_human_readable(preferences["alt_dl_limit"], 0)
        if preferences["alt_dl_limit"] > -1
        else "无限制"
    )
    fdict["alt_speed_up"] = (
        u.get_human_readable(preferences["alt_up_limit"], 0)
        if preferences["alt_up_limit"] > -1
        else "无限制"
    )
    ...
    fdict["queueing_count_slow_torrents"] = (
        "关" if preferences["dont_count_slow_torrents"] else "开"
    )
    ...
    fdict["max_ratio_enabled"] = "开" if preferences["max_ratio_enabled"] else "关"
    fdict["max_ratio"] = preferences["max_ratio"]
    fdict["max_seeding_time_enabled"] = (
        "开" if preferences["max_seeding_time_enabled"] else "关"
    )
    fdict["max_seeding_time"] = preferences["max_seeding_time"]
    fdict["max_ratio_act"] = (
        "暂停" if preferences["max_ratio_act"] == 0 else "删除"
    )  # 0 = pause them, 1 = remove them
...
updater.add_handler(
    ...
    bot_command=BotCommand("transferinfo", "BitTorrent 设置概览"),
)
updater.add_handler(
    CallbackQueryHandler(on_refresh_button_speed, pattern=r"^refreshtransferinfo$")
)
