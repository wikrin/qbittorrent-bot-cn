...
def manage_torrent_cb(update: Update, context: CallbackContext):
    ...
    update.callback_query.answer("使用 keyboard 管理 Torrent")
...
def see_trackers_cb(update: Update, context: CallbackContext):
    ...
    strings_list = [
        "<b>{status}:</b> {url} <b>({num_peers} peers)</b>".format(
            **{k: u.html_escape(str(v)) for k, v in tracker.items()}
        )
    ...
    update.callback_query.answer("Trackers 列表")
...
def refresh_torrent_cb(update: Update, context: CallbackContext):
    ...
    if not torrent:
        update.callback_query.edit_message_text(
            f"未找到 hash 为: <code>{torrent_hash}</code> 的Torrent",
            parse_mode=ParseMode.HTML,
        )
        return
    ...
    update.callback_query.answer("刷新 Torrent 信息")
...
def resume_torrent_cb(update: Update, context: CallbackContext):
    ...
    update.callback_query.answer("已恢复")
...
def toggle_atm_cb(update: Update, context: CallbackContext):
    ...
    update.callback_query.answer(
        f'自动 Torrent 管理 {"已禁用" if atm_status else "已启用"}'
    )
...
def force_resume_torrent_cb(update: Update, context: CallbackContext):
    ...
    update.callback_query.answer("强制继续")
...
def unforce_start_torrent_cb(update: Update, context: CallbackContext):
    ...
    update.callback_query.answer("强制继续 已取消")
...
def recheck_cb(update: Update, context: CallbackContext):
    ...
    update.callback_query.answer("开始 重新校验")
...
def no_notification_cb(update: Update, context: CallbackContext):
    ...
    if not config.notifications.no_notification_tag:
        update.callback_query.answer("tag 未在配置文件中设置", cache_time=10)
        return
...
def confirm_delete_with_files_cb(update: Update, context: CallbackContext):
    ...
    update.callback_query.edit_message_text(f"{torrent.name} 已删除")
...
def reduce_buttons(update: Update, context: CallbackContext):
    ...
    update.callback_query.answer("返回上级菜单")


updater.add_handler(
    MessageHandler(Filters.regex(r"^\/start info(.*)$"), on_info_deeplink)
)
...
updater.add_handler(
    CallbackQueryHandler(force_resume_torrent_cb, pattern=r"^forceresume:(.*)$")
)
updater.add_handler(
    CallbackQueryHandler(force_start_torrent_cb, pattern=r"^forcestart:(.*)$")
)
updater.add_handler(
    CallbackQueryHandler(unforce_start_torrent_cb, pattern=r"^unforcestart:(.*)$")
)
updater.add_handler(CallbackQueryHandler(recheck_cb, pattern=r"^recheck:(.*)$"))
updater.add_handler(
    CallbackQueryHandler(no_notification_cb, pattern=r"^nonotification:(.*)$")
)
updater.add_handler(
    CallbackQueryHandler(
        ask_confirm_delete_with_files_cb, pattern=r"^deletewithfiles:(.*)$"
    )
)
updater.add_handler(
    CallbackQueryHandler(
        confirm_delete_with_files_cb, pattern=r"^confirmdeletewithfiles:(.*)$"
    )
)
