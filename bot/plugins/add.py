...
def notify_addition(
    current_chat_id: int, bot: Bot, user: User, torrent_description: str
):
    ...
    ):  # do not send if the target chat is the current chat
        return

    text = (
        f"User {escape(user.full_name)} [<code>{user.id}</code>] added a torrent: "
        f"<code>{escape(torrent_description)}</code>"
    )
    bot.send_message(
        target_chat_id, text, parse_mode=ParseMode.HTML, disable_web_page_preview=True
    )
...
def add_from_magnet(update: Update, context: CallbackContext):
    ...
    update.message.reply_html(
        "添加了 Magnet", reply_markup=kb.short_markup(torrent_hash), quote=True
    )

    notify_addition(
        update.effective_chat.id, context.bot, update.effective_user, torrent_hash
    )
...
def add_from_file(update: Update, context: CallbackContext):
    logger.info(
        "application/x-bittorrent document from %s", update.effective_user.first_name
    )

    document = update.message.document
    if (
        document.mime_type != "application/x-bittorrent"
        and not document.file_name.lower().endswith(".torrent")
    ):
        logger.info(
            "invalid document from %s (mime type: %s; file name: %s)",
        ...
        update.message.reply_markdown(
            "请给我发一个有效的torrent文件 (`.torrent` 扩展名或 `application/x-bittorrent` MIME 类型)",
            quote=True,
        )
        return
    ...
    with open(file_path, "rb") as f:
        ...
        torrent_hash = hashlib.sha1(
            bencoding.bencode(decoded_dict[b"info"])
        ).hexdigest()
    ...
    update.message.reply_text(
        "添加了 Torrent", quote=True, reply_markup=kb.short_markup(torrent_hash)
    )
...
updater.add_handler(
    MessageHandler(Filters.text & Filters.regex(r"^magnet:\?.*"), add_from_magnet)
)
updater.add_handler(
    MessageHandler(
        Filters.text & Filters.regex(r"^https?:\/\/.*(jackett|\.torren|\/torrent).*"),
        add_from_url,
    )
)
