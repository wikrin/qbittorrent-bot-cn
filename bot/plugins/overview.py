...
from qbt.custom import STATES_DICT, TORRENT_STRING_COMPACT
...
• <b>torrent 状态:</b> {states_count}
• <b>分类:</b> {categories_count}

➤ <b>上传中 ({active_up_count}):</b>
{active_up}

➤ <b>下载中 ({active_down_count}):</b>
{active_down}
...
def get_quick_info_text(sort_active_by_dl_speed=True):
    ...
    active_torrents_down = [
        torrent for torrent in all_torrents if torrent.state in ("downloading",)
    ]
    active_torrents_down.sort(key=lambda t: t["dlspeed"])

    active_torrents_down_strings_list = ["没有正在下载的 torrents"]
    active_torrents_up_strings_list = ["没有正在上传的 torrents"]
    ...
    if active_torrents_down:
        active_torrents_down_strings_list = [
            TORRENT_STRING_COMPACT.format(**t.dict()) for t in active_torrents_down
        ]
    if active_torrents_up:
        active_torrents_up_strings_list = [
            TORRENT_STRING_COMPACT.format(**t.dict()) for t in active_torrents_up
        ]
    ...
    for category, count in categories_counter.most_common():
        categories_count_list.append(f"{count} {category or '未分类'}")
    if categories_count_list:
        categories_count_string = ", ".join(categories_count_list)

    schedule_info = qb.get_schedule()
    if not schedule_info:
        schedule_string = "<b>计划限速</b>: off"
    else:
        schedule_string = (
            "<b>计划限速</b>: on, 从 {from_hour} 到 {to_hour} ({days})".format(
                **schedule_info
            )
        )
    ...
    if alt_speed_info["status"] == "on":
        current_speed_string = (
            f'▲ <b>{current_speed[1]}/s</b> ({alt_speed_info["alt_up_limit"]}/s)\n'
        ...
        current_speed_string = (
            f"▲ <b>{current_speed[1]}/s</b>{up_limit}\n"
...
def on_overview_command(update: Update, context: CallbackContext):
    ...
    sent_message = update.message.reply_html(
        text, reply_markup=kb.get_overview_base_markup()
    )
...
def on_refresh_button_overview(update: Update, context: CallbackContext):
    ...
    update.callback_query.edit_message_text(
        text, parse_mode=ParseMode.HTML, reply_markup=kb.get_overview_base_markup()
    )
...
def on_alton_button_overview(update: Update, context: CallbackContext):
    ...
    text = get_quick_info_text()
    update.callback_query.edit_message_text(
        text, parse_mode=ParseMode.HTML, reply_markup=kb.get_overview_altspeed_markup()
    )
    update.callback_query.answer("速度限制已启用")
...
def on_altoff_button_overview(update: Update, context: CallbackContext):
    ...
    update.callback_query.answer("速度限制已关闭")
...
def on_transfer_info_button_overview(update: Update, context: CallbackContext):
    ...
    update.callback_query.answer("正在显示传输详情...")
    update.callback_query.edit_message_text(
        text, parse_mode=ParseMode.HTML, reply_markup=kb.get_overview_base_markup()
    )
...
def on_manage_alt_speed_button_overview(update: Update, context: CallbackContext):
    ...
    update.callback_query.answer("使用按钮配置您的备用速度")
...
def on_manage_schedule_button_overview(update: Update, context: CallbackContext):
    ...
    update.callback_query.answer("使用按钮配置您的计划限速")
...
def on_schedon_button_overview(update: Update, context: CallbackContext):
    ...
    text = get_quick_info_text()
    update.callback_query.edit_message_text(
        text, parse_mode=ParseMode.HTML, reply_markup=kb.get_overview_schedule_markup()
    )
    update.callback_query.answer("计划限速已开启")
...
def on_schedoff_button_overview(update: Update, context: CallbackContext):
    ...
    update.callback_query.answer("计划限速已关闭")


updater.add_handler(
    CommandHandler(["overview", "ov"], on_overview_command),
    bot_command=BotCommand("overview", "活跃总览"),
)
...
updater.add_handler(
    CallbackQueryHandler(
        on_refresh_button_overview, pattern=r"^(?:quick|overview):refresh:(\w+)$"
    )
)
updater.add_handler(
    CallbackQueryHandler(on_free_space_button_overview, pattern=r"^overview:freespace")
)
updater.add_handler(
    CallbackQueryHandler(
        on_transfer_info_button_overview, pattern=r"^overview:transferinfo"
    )
)
updater.add_handler(
    CallbackQueryHandler(
        on_manage_alt_speed_button_overview, pattern=r"^overview:altspeed$"
    )
)
updater.add_handler(
    CallbackQueryHandler(
        on_manage_schedule_button_overview, pattern=r"^overview:schedule$"
    )
)
updater.add_handler(
    CallbackQueryHandler(
        on_alton_button_overview, pattern=r"^(?:quick|overview):alton$"
    )
)
updater.add_handler(
    CallbackQueryHandler(
        on_altoff_button_overview, pattern=r"^(?:quick|overview):altoff$"
    )
)
updater.add_handler(
    CallbackQueryHandler(
        on_schedon_button_overview, pattern=r"^(?:quick|overview):schedon"
    )
)
updater.add_handler(
    CallbackQueryHandler(
        on_schedoff_button_overview, pattern=r"^(?:quick|overview):schedoff"
    )
)
